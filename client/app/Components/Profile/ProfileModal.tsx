...
import { badge, check, mail, github } from '@/utils/Icons';
...
function ProfileModal() {
  ...
  const handlePassword = (type: string) => () => (e: React.ChangeEvent) => {
    if(type === "old") {
        ...
        >
          ...
          <div className='mt-4 relative flex justify-between'>
              <div className='relative inline-block'>
                ...
                <div className='absolute bottom-0 right-1 rounded-full border-0' >
                    <span className='text-lg text-blue-400'>{badge}</span>
                    <span className='absolute z-20 left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] text-xs text-white'>{check}</span>
                </div>
              </div>
          </div>
          <div className='self-end flex items-center gap-2'>
            <button className='flex items-center gap-2 border-2 border-[#323232]/10 rounded-md py-1 px-3 text-xs font-medium text-[#323232]'>{github} Github</button>
